...
def test_milestone11_express_balanced_has_no_critical_path_or_authz_gap(tmp_path: Path) -> None:
    repo_path = Path(__file__).resolve().parents[1] / "eval" / "repos" / "milestone11_express_balanced"
    ...
    assert "critical_write_missing_authz" not in rule_ids
