...
def test_milestone19_django_integrity_gap_flags_stage14_rules(tmp_path: Path) -> None:
    repo_path = Path(__file__).resolve().parents[1] / "eval" / "repos" / "milestone19_django_integrity_gap"
...
def test_milestone19_django_integrity_balanced_avoids_stage14_rules(tmp_path: Path) -> None:
    repo_path = Path(__file__).resolve().parents[1] / "eval" / "repos" / "milestone19_django_integrity_balanced"
    out_dir = tmp_path / ".riskmap"
